...
describe("the outcome table", () => {
  it("covers every outcome the attempt policy knows about", () => {
  ...
  it("every mapped destination is a real state", () => {
  ...
  it("a wrong number is not recorded as having reached the business", () => {
...
describe("an opt-out suppresses the business permanently", () => {
  it("suppresses, then transitions, and the business ends terminal", () => {
    const { rec, suppression } = recorder();
    const result = rec.record({ prospect: prospect("connected"), outcome: "opt_out", ...base, note: "Asked never to be called again.", e164: "+61355501042" });
  ...
  it("catches the business on a number we have never seen before", () => {
    // The whole reason an opt-out is business-scoped: they must not be
    // reachable on their other line next week.
    const { rec, suppression } = recorder();
    rec.record({ prospect: prospect("connected"), outcome: "opt_out", ...base, note: "Do not call again.", e164: "+61355501042" });
    assert.strictEqual(suppression.check({ e164: "+61491570999", fingerprint: FINGERPRINT }).suppressed, true);
  });

  it("refuses to record an opt-out it cannot act on", () => {
    ...
    const result = rec.record({ prospect: prospect("connected"), outcome: "opt_out", ...base, note: "Do not call again." });
    assert.strictEqual(result.ok, false);
    assert.strictEqual(result.code, "suppression_unavailable");
    assert.match(result.message, /would leave them callable/);
  });

  it("does not transition the prospect if the suppression write fails", () => {
    const failing = {
    ...
    const result = rec.record({ prospect: p, outcome: "opt_out", ...base, note: "Do not call again." });
  ...
  it("the suppression survives the prospect record being thrown away", () => {
    ...
    const { rec, suppression } = recorder();
    rec.record({ prospect: prospect("connected"), outcome: "opt_out", ...base, note: "Do not call again." });
  ...
  it("is audited as a contact outcome with the effect it had", () => {
...
describe("a wrong number suppresses the number only", () => {
  it("suppresses the number and leaves the business reachable elsewhere", () => {
    const { rec, suppression } = recorder();
    const result = rec.record({ prospect: prospect("attempted"), outcome: "wrong_person", ...base, note: "Reached a private residence.", e164: "+61355501042" });
  ...
  it("needs the number that was dialled", () => {
    const { rec } = recorder();
    const result = rec.record({ prospect: prospect("attempted"), outcome: "wrong_person", ...base, note: "Reached a private residence." });
    assert.strictEqual(result.ok, false);
    assert.strictEqual(result.code, "number_required");
  });

  it("does not claim we spoke to the business", () => {
    const { rec } = recorder();
    const result = rec.record({ prospect: prospect("queued"), outcome: "wrong_person", ...base, note: "Reached a private residence.", e164: "+61355501042" });
...
describe("the prospect is walked through states that actually happened", () => {
  it("a queued prospect that nobody answers becomes attempted", () => {
    const { rec } = recorder();
    const result = rec.record({ prospect: prospect("queued"), outcome: "no_answer", ...base, note: "Rang out." });
    assert.strictEqual(result.to, "attempted");
    assert.deepStrictEqual(result.hops.map((h) => h.to), ["attempted"]);
  });

  it("a queued prospect that declines is recorded as attempted, then connected, then not interested", () => {
    // The history should read "we called, we spoke to them, they said no" —
    // not "we called, they said no".
    const { rec } = recorder();
    const result = rec.record({ prospect: prospect("queued"), outcome: "not_interested", ...base, note: "Happy with their current setup." });
  ...
  it("every hop records who and why", () => {
    const { rec } = recorder();
    const result = rec.record({ prospect: prospect("queued"), outcome: "callback", ...base, note: "Asked for Tuesday." });
  ...
  it("an outcome that would need an illegal hop is refused, and nothing moves", () => {
    const { rec } = recorder();
    ...
    assert.strictEqual(rec.record({ prospect: p, outcome: "booked", ...base, note: "Booked a demo." }).ok, true);
    assert.strictEqual(p.lifecycle, "callback_requested", "the original must be untouched — prospects are frozen");
  });

  it("refuses an outcome for a business no call could have reached", () => {
    const { rec } = recorder();
    for (const lifecycle of S.PROSPECT_STATES.filter((s) => !OUTCOME_RECORDABLE_FROM.includes(s))) {
      const result = rec.record({ prospect: prospect(lifecycle), outcome: "no_answer", ...base, note: "Rang out." });
  ...
  it("a suppressed business cannot have an outcome recorded against it at all", () => {
    const { rec } = recorder();
    const result = rec.record({ prospect: prospect("suppressed"), outcome: "booked", ...base, note: "They changed their mind." });
    assert.strictEqual(result.ok, false);
    assert.strictEqual(result.code, "not_contactable_state");
  });
});
...
describe("consequences nobody has approved are stated as such", () => {
  it("a not-interested cooldown says out loud that its duration is not agreed", () => {
    const { rec } = recorder();
    const result = rec.record({ prospect: prospect("connected"), outcome: "not_interested", ...base, note: "Not right now." });
  ...
  it("records the outcome anyway — the record of what they said is not optional", () => {
    const { rec } = recorder();
    const result = rec.record({ prospect: prospect("connected"), outcome: "declined", ...base, note: "Declined." });
    assert.strictEqual(result.ok, true);
    assert.strictEqual(result.prospect.lifecycle, "not_interested");
  });

  it("an approved policy reports the cooldown as in force", () => {
    const suppression = createSuppressionList({ now });
  ...
  it("the two approved suppressions are applied regardless of the policy's approval state", () => {
    // Opt-out and wrong-number are statutory/settled (§5 G5, §9); they do not
    // wait on the caps and cooldowns being agreed.
    const { rec, suppression } = recorder();
    assert.strictEqual(rec.record({ prospect: prospect("connected"), outcome: "opt_out", ...base, note: "Do not call." }).ok, true);
...
describe("validation", () => {
  it("refuses an unknown outcome rather than guessing", () => {
    const { rec } = recorder();
    for (const outcome of ["interested", "CONNECTED", "", null, undefined, 42]) {
      const result = rec.record({ prospect: prospect(), outcome, ...base });
      assert.strictEqual(result.ok, false, `"${outcome}" should not be accepted`);
      assert.strictEqual(result.code, "outcome_unknown");
    }
  });

  it("requires who observed it and what happened", () => {
    const { rec } = recorder();
    assert.strictEqual(rec.record({ prospect: prospect(), outcome: "no_answer", actor: null, note: "x" }).code, "actor_missing");
    assert.strictEqual(rec.record({ prospect: prospect(), outcome: "no_answer", actor: "Peter", note: "  " }).code, "note_missing");
  });

  it("refuses a malformed prospect rather than throwing", () => {
    const { rec } = recorder();
    for (const bad of [null, undefined, "x", 7, []]) {
      assert.strictEqual(rec.record({ prospect: bad, outcome: "no_answer", ...base }).code, "prospect_invalid");
    }
  });
...
describe("becoming a client", () => {
  it("an interested prospect can convert, and leaves the prospecting pool", () => {
    const { rec } = recorder();
  ...
  it("converting a business that said no needs an administrative decision", () => {
    const { rec } = recorder();
  ...
  it("a suppressed business can never become a client through this path", () => {
    const { rec } = recorder();
  ...
  it("requires who confirmed it and what was agreed", () => {
    const { rec } = recorder();
