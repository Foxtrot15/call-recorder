...
select
  'acquisition purposes permitted' as check,
  count(*)                         as found,
...
select
  'total rows' as check,
  count(*)     as found,
  'LPM4 creates no row — an acquisition agent appears only when one is provisioned' as verdict
from public.provider_resources;

select
  'acquisition rows' as check,
  count(*)           as found,
