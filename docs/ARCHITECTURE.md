...
| `/client/locksmith-onboarding/:sessionId/*` | routes/locksmith-onboarding.js | client | Locksmith onboarding review + approval for a **voice-originated** session (M2). **Dormant**: 404s unless `LOCKSMITH_ONBOARDING_ENABLED="true"`. See [LOCKSMITH_ONBOARDING_SPEC.md](LOCKSMITH_ONBOARDING_SPEC.md) |
| `/client/locksmith-setup/*` | routes/locksmith-setup-handlers.js | client | The setup wizard a locksmith fills in themselves (M8A): seven steps, save/resume, review, per-section confirmation, approval, version history, rollback, test checklist, go-live gates. Writes only to a `status:"draft"` profile row, so nothing live or under review is reachable. Dormant with the same flag. See [LOCKSMITH_SETUP_JOURNEY_SPEC.md](LOCKSMITH_SETUP_JOURNEY_SPEC.md) |
