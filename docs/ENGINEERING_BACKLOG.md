...
| P1-6 | **Personal-contacts feature silently broken — table doesn't exist** — ✅ **RESOLVED 2026-07-06**: `create_personal_contacts.sql` applied (table + unique constraint + RLS), post-apply smoke green; the feature now works with the deployed code. Fail-closed error handling added on this branch so a recurrence can't be silent. | `services/personal-filter.js` + `routes/personal-contacts.js` + `recording.js:143` + `index.html:1226` | Was: the table was never created (proven by RLS apply error 42P01) and the service discarded every Supabase error, so the UI's privacy promise ("Aida won't record future calls from this number") was confirmed with `{success:true}` while nothing was saved, and flagged callers were still recorded/transcribed/emailed. See the dedicated investigation below. | High — proven by DB error |
...
| P2-2 | **Duplicated Claude email-drafting block** — ✅ **RESOLVED** on `fix/llm-prompt-contamination`: `services/followup.js` extracted exactly as proposed; both routes now share one grounded, mode-aware, guard-validated implementation (see `docs/LLM_PROMPT_CONTAMINATION_INVESTIGATION.md`). | ~~`routes/recording.js` vs `routes/test.js`~~ | The drift was a contributing cause of the hallucinated-draft incident. |
