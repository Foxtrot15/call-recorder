...
| **E-7B1** | Durable dispatch authority + durable emergency stop | **DESIGN READY — SQL AWAITING FOUNDER APPROVAL.** No `.sql` file exists, nothing applied, no dev row written. Two tables proposed (`acquisition_dial_executions`, `acquisition_calling_state`), and the review **corrected the earlier runbook §14.3 sketch**: `authorisation_id` is a derived hash that legitimately collides, and uniqueness on it would not have stopped two workers each minting their own authorisation for the same business. Full design and the exact SQL: [ACQUISITION_E7B1_DESIGN.md](ACQUISITION_E7B1_DESIGN.md). **After that migration, calling is still `paused` by default and no live provider exists.** |
