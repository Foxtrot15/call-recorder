...
| ~~**E-9**~~ | One Retell `call_id` must belong to at most one dispatch | **CLOSED ON DEV 2026-08-13 — `laq6` applied by hand, verifier 8/8 PASS. Production: NOT APPLIED.** Pre-migration the verifier reported **6/8**, failing exactly the two E-9 checks and nothing else — evidence it discriminates rather than always passing. The rollback mutation-probe script then **completed without SQL error**; the editor did not surface its per-case `RAISE NOTICE` lines, so **the six individual probe results were not visually observed by anybody**, and that is recorded rather than dressed up. **Zero permanent proof residue: dev is 23 before and after.** No live two-process race was performed and none is claimed — see §15 of the design doc for the deliberate choice. It enforces **two** invariants, because the founder's revision caught that one is insufficient: a unique index cannot catch two workers who both read NULL and want **different** references — R1 and R2 collide with nothing. So **A**: `unique (provider_ref) where provider_ref is not null`, deliberately **not** partial on `resolved_at`; and **B**: `provider_ref` is **write-once** in the laq5 guard, refusing `R1→R2` **and `R1→NULL`** — without the second, a caller could clear a reference and rebind it elsewhere while every statement satisfied the index. Key is `provider_ref` alone, not `(provider, provider_ref)`: the composite is only sound if `provider` is also sticky, and `provider` is not writable by any path. `fail()` now preserves `code`/`constraint`, without which a permanent conflict reached operators as a transient `store_unavailable`. New `call_id_taken` distinguishes "another dispatch owns this call" from "this dispatch holds a different one". 25 offline proofs; **no live two-process race was run and none is claimed**. [Design](ACQUISITION_E9_CALL_BINDING_DESIGN.md) · [runbook §16](ACQUISITION_SQL_RUNBOOK.md) |
| ~~**E-9 (Phase 1)**~~ | Superseded by the row above | **Phase 1 audit — DESIGN READY.** The audit found more than the known non-atomicity: for **two different dispatches claiming the same `call_id` there is no application check at all** — the code inspects only the current dispatch's own `provider_ref`. `provider_ref` is also absent from the laq5 guard's immutable list, so `R1 → R2` is refused by application code and by nothing else. Not reachable from genuine Retell traffic (the binding key is metadata we set, and one call carries one metadata object), but that is an argument about likelihood, not correctness. Proposed: `unique (provider_ref) where provider_ref is not null` — **not partial on `resolved_at`**, unlike laq5's other two, because a business may be dispatched again but a `call_id` may never be rebound. **The migration alone is insufficient**: `updateDialExecution` wraps errors through `fail()` and discards `code`/`constraint`, so a violation would surface as a transient-sounding `store_unavailable`. Dev pre-flight is clean (1 row, `provider_ref` NULL, no duplicates). A true two-process proof needs a second permanent row **and** mutating the approved proof row — not proposed without a decision. [ACQUISITION_E9_CALL_BINDING_DESIGN.md](ACQUISITION_E9_CALL_BINDING_DESIGN.md) |
| **E-10A** | The acquisition agent, specified | **LOCAL SPEC COMPLETE — AGENT NOT PROVISIONED, no network write.** `acquisition-agent-spec.js` builds the opening, the `general_prompt`, the Retell `post_call_analysis_data` fields and the exact `create-agent` payload — and sends none of it. **AI disclosure is unprompted in the opening and answered plainly if asked**, recorded as founder product policy and **not** as a legal requirement. **Identity model settled**: `assistantName` Aida, `companyName` **Niche Drops** (the business placing the call), `productName` **AIDA** (the product discussed) — three separate fields, never merged, with the prompt stating each role so the agent cannot present the product as the caller (§19.2). Ratchets test **meaning, not copy** — four rewordings accepted, seven meaning-losses rejected (§19.2a). Two further findings: the marketing tagline *"Never lose another after-hours locksmith enquiry"* is an **absolute** and is **forbidden as a spoken claim** (§19.3); and **price is never hardcoded or raised first** because config declares it provisional with exactly one numeric source (§19.4). **Voicemail: leave no message** — it consumes a counted attempt under A-L7, no message has ever been reviewed, and machine detection has never been observed working, so no template is invented (§19.5). 37 offline proofs including eighteen conversations and ratchets whose teeth were checked by planting a guarantee. [Design §19](ACQUISITION_E7B2_RETELL_DESIGN.md) |
| **E-10C** | The acquisition Retell resource split | **COMPLETE, OFFLINE — NEITHER RESOURCE PROVISIONED.** Fixes the E-10B defect: the compiler described **one** object carrying agent fields *and* `general_prompt` *and* `begin_message`, which would have created **an agent with no brain** (`llm_id` null) while the prompt went to an endpoint that does not accept one. Now two resources in this repository's own established shape — engine (`create-retell-llm`) carries prompt, opening, dynamic variables and an **empty** tool list; agent (`create-agent`) carries name, `response_engine{type,llm_id}`, voice, language, webhook and analysis — with the dependency **expressed** (`dependsOn: response_engine`) rather than remembered. **Acquisition is deliberately NOT in `DESIRED_RESOURCE_ORDER`**: that list is keyed by `resourceType` and resolved against one compiled receptionist, so an acquisition entry would have been handed the **receptionist's** payload under an acquisition purpose. Tests prove `provisioning-plan.js` never mentions acquisition and that **nothing anywhere constructs these resources**. Readiness is **computed**, not hardcoded, and names its blockers: no engine id, voice unresolved, webhook unresolved, **answering-machine behaviour unverified** *(that fourth blocker was **superseded by E-12A**, which found it unsatisfiable by construction and split it into configured-vs-observed)*. 23 offline proofs. [Design §20](ACQUISITION_E7B2_RETELL_DESIGN.md) |
| **E-10D(i)** | The acquisition response engine | **PROVISIONED on dev Retell, 2026-08-13 — the first authorised network write in this chain.** One `createResponseEngine` request, no retry, `dev` tag only. A Retell LLM holding a prompt and an opening: **no voice, no agent, no number, no webhook — it cannot ring anybody and cannot be rung.** Two things had to be built to do it: a **transport**, supplied for one hand-run command rather than wired into the service (nothing in the branch injects a `fetchImpl`, which is what has kept it structurally incapable of a live write); and a **narrowed ratchet** — the rule that acquisition is never provisioned as a side effect is unchanged, but the exception is now **one named filename** rather than a relaxed pattern. That no agent was created is known **structurally**: four ratchets read the only file that can reach Retell and assert exactly one `adapter.create*` call, no agent/number/call path, no retry, and that the agent payload is unreachable. `RETELL_ACQUISITION_LLM_ID` is set in the deployment environment, **not in git**. [Design §21](ACQUISITION_E7B2_RETELL_DESIGN.md) |
| ~~**E-11A**~~ | The acquisition webhook ingress | **CLOSED ON DEV — BUILT AND DORMANT — mounted in source, NOT deployed, NOT exposed.** `POST /webhooks/retell/acquisition`, separate from the onboarding ingress because Retell attaches `webhook_url` **per agent** and `decideEventHandling` is onboarding-shaped (it answers `record_unbound` when no onboarding session matches, so acquisition events would be recorded and never processed). Reuses the **one** signature verifier — a ratchet asserts this handler contains no `crypto`/`createHmac` — and the LPM3 fingerprint store; reuses neither the onboarding decision logic nor its processor. Behind a **third** flag, `RETELL_ACQUISITION_WEBHOOK_ENABLED`, so enabling onboarding webhooks can never enable acquisition ingestion. The "is it ours?" check sits **before** the fingerprint write, so another product's traffic never fills our event log. A **permanent** call-id conflict answers **204**, not 5xx — Retell retries non-2xx, and a conflict refused identically for ever would become a stream of redeliveries; it is logged `failed` for a human instead. **503 is reserved for genuine storage outage.** 33 offline proofs. **LPM3 is now APPLIED TO DEV** (founder, 2026-08-13) so the durable idempotency schema is present — `provider_webhook_events` with UNIQUE `pwe_fingerprint_key` is the arbitration authority, and **production has not received it**. See [runbook §17](ACQUISITION_SQL_RUNBOOK.md). [Design §22](ACQUISITION_E7B2_RETELL_DESIGN.md) |
| ~~**E-12A**~~ | The answering-machine policy, as a provider setting | **CLOSED — CONFIGURED LOCALLY, NOT LIVE-VERIFIED. No Retell request, no SQL, no DEV write.** Founder policy is **hang up on detected voicemail, leave no message**, and the finding of this milestone is that it was previously carried by **nothing that reaches the provider**: `VOICEMAIL_POLICY` has existed since E-10A, but it is a spec object read by documents and tests, and `buildAcquisitionAgentPrompt` **never emitted a word of it into `general_prompt`** — asserted, not assumed. So this was not weak enforcement, it was none, and the cost of losing was specific: under A-L7 a voicemail spends **one of only two counted attempts** on a recording nobody agreed to receive. The acquisition agent now carries `voicemail_option: { action: { type: "hangup" } }` — Retell ends the call itself, without consulting the model. It is a **frozen module constant, deliberately not a config field**: every other field on that payload falls back to a caller-supplied value, and a caller able to supply the action could supply a static-text one and turn "leave no message" into a message from outside the file where the policy is reviewed. Six hostile config shapes are fed in and the payload is asserted unmoved. **Acquisition only** — `voicemail_option` is emitted from exactly one file in `src/`, proven by a directory walk, and the receptionist's **actually compiled** payload is built in the test and inspected. **The prompt was deliberately NOT changed, and that is now DECIDED, not open**: `general_prompt` belongs to the response engine, which is **already provisioned**, so editing it would desynchronise local spec from the live resource and require an `updateResponseEngine` network write. **Founder, 2026-08-13: leave the engine unchanged** — machine detection is the provider's, the prompt is not the detection authority, and defence-in-depth wording alone does not justify a live update. Verified byte-identical: the engine built at this commit and at `31075f2` are the same object, same 5373-character prompt, same opening — **no drift**. Readiness now separates **configured locally: YES** (computed by reading the payload) from **observed on a real call: NO** (hardcoded `false`, because nothing here can observe a call). This also repaired an **unsatisfiable gate**: "answering-machine behaviour unverified" was a create-agent blocker that could never be cleared, since the behaviour cannot be seen until an agent exists and has been rung; it moved to `unverifiedAfterCreation` — **reported, not retired**. `createAgentReady` is still **false**: voice and webhook remain unresolved. **Blocker correction (founder, 2026-08-13):** creating the agent *resource* requires only the `llm_id` wired into provisioning, a selected `voice_id`, and a `webhook_url` while the policy is "create it correctly once". The outbound **number**, **DNCR-1**, **A-L2**, a live transport and enabling calling are **pre-live-call** gates, not agent-creation prerequisites — an agent with no number and paused calling cannot ring anybody. **DNCR-1, A-L2 and E-7 all remain OPEN.** Outcome and attempt semantics **unchanged** — `voicemail_reached`/`machine_detected` → `voicemail`, still a counted attempt, `no_answer` still uncounted, no retry, no callback. **IVR and `call_screening_option` deliberately NOT enabled** (screening is noted as a plausible later optimisation for cold calling). 29 offline proofs. [Design §23](ACQUISITION_E7B2_RETELL_DESIGN.md) |
| ~~**E-12B**~~ | Response-engine drift pin + voice catalogue read | **CLOSED — ONE READ-ONLY RETELL REQUEST, ZERO WRITES. No voice selected.** Two things. **(1) The pin.** E-10D(i) created a real response engine from this repository's own payload, so the prompt stopped being a local file and became a claim about a remote resource: editing `general_prompt` changes nothing at Retell and nothing fails, and the first place anybody notices is a call to a stranger. `PROVISIONED_RESPONSE_ENGINE.payloadHash` = `b0b5e21e…0542`, computed with the repo's **existing** `payloadHash`/`stableStringify` from `voice-platform-port.js` rather than a new convention. The exact provisioned payload is **reproducible** — E-10D(i) sent `describeAcquisitionRetellResources().responseEngine` with **no arguments**, so it hashes identically at `d591262`, `31075f2` and today. It does **not** freeze the copy; it forbids moving it *silently*, and the failure message names the missing third step (update the remote engine, re-pin in the same commit). **Proven by planting real drift in the source** — two words added to `reasonForCalling` failed 8 assertions — then reverted. Scoped both ways: choosing a `voice_id`, setting `webhook_url`, supplying the `llm_id` and the E-12A `voicemail_option` are **agent** fields and do not move it, which matters because resolving the voice is the next milestone and must not read as prompt drift. The `llm_id` is **not** in the pin — a ratchet asserts no `llm_…` value is in tracked source. 17 offline proofs. **(2) The catalogue.** One `GET /list-voices`, HTTP 200, **272 voices**, run from an uncommitted operational script — a founder-review action, not a pipeline step, and committing it would add a second Retell-capable file to the surface the safety story rests on. **Only 2 of 272 voices carry Australian accent metadata, and both are male** (`11labs-Noah`, `11labs-charlie`, both ElevenLabs). **There is no female preset with Australian accent metadata** — a real constraint, counted across the whole catalogue, not a gap in the search. The existing receptionist default `custom_voice_018b4225…` **is** present as *"Sunny - Australian Female"* but carries **no accent, gender or age metadata at all**, so its Australianness rests on its name — the exact inference this milestone was told not to make. Reported, not selected — **and then selected by the founder after audition: "Sunny - Australian Female", `voice_type: custom`, ElevenLabs.** The accent claim is recorded as **founder audition evidence**, not provider metadata, because Retell returned none. **(3) The config boundary.** The value lives in a NEW acquisition-only key, `RETELL_ACQUISITION_VOICE_ID`, deliberately **not** the shared `RETELL_DEFAULT_VOICE_ID` — which currently holds the same voice, and that coincidence is exactly the trap: had acquisition read it, re-voicing the receptionist would silently re-voice every cold call to a stranger with no test failing. `resolveAcquisitionVoiceId` has **no fallback**; an unset key leaves acquisition **voiceless** rather than borrowed, keeps `createAgentReady` false, and names the variable and the chosen voice in the blocker. The id is **not committed** — a ratchet asserts it appears in neither the spec nor the config module. Selecting a voice does **not** touch the pinned engine (`voice_id` is an agent field), so no `updateResponseEngine` was required. **Voice is no longer a create-agent blocker; `webhook_url` is the only one left.** 16 further offline proofs. [Design §24](ACQUISITION_E7B2_RETELL_DESIGN.md) |
| **E-12C** | Acquisition webhook public exposure | **STOPPED AT THE DEPLOYMENT GATE — NOTHING DEPLOYED, NO URL, NO RETELL REQUEST. Awaiting ONE founder decision.** The goal was a real public HTTPS URL for `POST /webhooks/retell/acquisition` so the agent can be created correctly once. The audit ended it: **there is exactly one environment and it is production.** Authority, not inference — `DEPLOYMENT.md` says Railway auto-deploys `main`; `PRE_REACT_NATIVE_REVIEW.md` §M2 says *"Today there is exactly one environment: production"*; `LOCKSMITH_PILOT_SPEC.md` §8 says *"nothing is deployed and there is no staging target"*; CI's smoke job **self-skips** because no `SMOKE_BASE_URL` exists; and no Render/Fly/Vercel/Procfile/Dockerfile/Railway manifest is tracked. A non-production runtime therefore needs **a second Railway service or a tunnel** — new material infrastructure, and a founder decision. Both stop conditions in the brief applied. **Deploying acquisition code into the production runtime to obtain a hostname was never an option**: that runtime answers real customer calls. Worth recording that exposure would publish **the whole Aida server**, not a route — operator login, client auth and the Twilio webhooks share the process (all signature- or auth-gated; locksmith/onboarding/portal/VoIP/both Retell webhooks stay flag-gated dormant). The safety property was proven rather than assumed: with **all three ingress flags on and an API key present**, `canWriteLive` and `canPlaceCall` are both **refused**, `isAcquisitionEnabled` is **false**, every provider stays `live:false`, dry-run stays **on**, and `acquisitionReady("dial")` answers `acquisition_disabled` — opening the door grants nothing that walks out of it, and each of the three flags is individually load-bearing. No signature bypass exists to find later: no `NODE_ENV` development branch, no `allowUnsigned` flag, no query-string secret, and an unavailable SDK **fails closed** with `verifier_unavailable` rather than improvising an HMAC. All three acquisition ids — `RETELL_ACQUISITION_LLM_ID`, `..._VOICE_ID`, `..._WEBHOOK_URL` — are **unset in every environment**, so `createAgentReady` is false for **three** reasons, not one. The E-12B webhook boundary was **verified, not redesigned**. 15 offline proofs. [Design §25](ACQUISITION_E7B2_RETELL_DESIGN.md) |
| ~~**E-12D**~~ | The acquisition webhook's durable wiring | **CLOSED LOCALLY — NOT PUSHED, NOT DEPLOYED, NO LIVE WEBHOOK EVER RECEIVED. No SQL, zero DEV writes.** E-11A built the ingress and wired it to nothing: the route built the handler with no dependencies, so `store` defaulted to `null` and a genuine signed event was verified, fingerprinted, acknowledged and then refused with `acquisition_event_store_unavailable`, `mutated:false` — fail-safe and incapable of finishing. `acquisition-webhook-deps.js` is ~30 lines of composition over three **existing** authorities (`createSupabaseAcquisitionStore`, `createDurableSuppression`, `createDurableOutcomes`): **no new table, no SQL, no second persistence model, no ad-hoc Supabase client**, and **no adapter needed** — the existing store already satisfied the handler's contract, which was checked rather than assumed. **The load-bearing decision is that composition is LAZY.** `createDurableSuppression` hydrates from the database at construction, and `server.js` imports this router unconditionally while **production has no acquisition schema** — composing at module load would have made *a module merely existing* into a production query. It is therefore built on first request, which the three-flag gate makes unreachable while dormant; a failed build is deliberately **not** memoised so a transient outage cannot disable the route for the process lifetime. `audit` stays null (the decision chain is hash-linked with its own contention handling, not a drop-in), and that weakens nothing: the recorder still refuses `suppression_unavailable`, so nothing is recorded as an opt-out unless the business genuinely becomes uncallable. Ordering, idempotency, exact-dispatch-id correlation, E-9 write-once binding, E-8 lifecycle and E-12A voicemail are all re-proven **against the wired composition** rather than a mock. Failure semantics stated explicitly: 401 auth · 400 malformed · 413 oversize · 204 not-ours/duplicate/**permanent** conflict · **503 transient only** (fingerprint outage, unbuildable durable layer) — never a 2xx that loses an event. One test-design correction found by the ratchets: an opt-out is a **flag on** an outcome, not an outcome value. 37 offline proofs. [Design §26](ACQUISITION_E7B2_RETELL_DESIGN.md) |
| ~~**E-12E–H**~~ | Pre-live acquisition stack, local only | **CLOSED LOCALLY — NOTHING PUSHED, NOTHING DEPLOYED, NO RETELL REQUEST, NO SQL APPLIED, ZERO DEV WRITES.** Four stages. **E-12E** the one-shot agent runner: preview by default, exactly one adapter call in the file, and refusals for the prod tag, engine drift, missing llm_id, missing/mismatched acquisition voice, a voicemail action that is not `hangup`, any voicemail text, a language other than `en-AU`, a webhook that is missing/not-https/local, and an already-recorded agent. `NODE_ENV=production` with `RETELL_ALLOWED_TAG=staging` is **accepted** — they describe different things. No retry: ambiguous exits 2 as UNKNOWN; created-but-unrecorded shouts the id three times and exits 3, because an agent that exists unrecorded is worse than the reverse. **E-12F** the one-agent authority, which turned out to already exist: LPM3’s `pr_one_active_per_purpose` is a **database** unique index, so nothing was built. The only obstacle was the `purpose` CHECK, widened by `lpm4_acquisition_provider_resources.sql` — **NOT APPLIED**, no new table, no data change, reversible, discovers the auto-generated constraint name. Until applied the app builds the row and Postgres refuses it, naming the migration in the error. Reconciliation is explicit: success-then-persistence-failure is `reconciliation_required`, forbids creating, asks for a human; an unreadable authority **throws**, because null would read as “no agent exists”. **E-12G** the number boundary: `canPlaceCall` is satisfied by `RETELL_OUTBOUND_ONBOARDING_NUMBER`, so acquisition gets `RETELL_ACQUISITION_OUTBOUND_NUMBER` with **no fallback** — otherwise cold calls to strangers would have gone out from the caller ID a consenting client was interviewed on. **E-12H** the founder preflight, whose `ready` field is **hardcoded false permanently** so it can never become a second, weaker authorisation path; the M8E pre-dial gate still runs and still owns every compliance decision. The proof authorisation is deliberately **not** `ALLOW_TEST_CALL=true` — that authorises a state, not an act, and stays true afterwards; it reuses the repo’s named-human-approver and canonical-identity-hash patterns, binds one prospect/destination/from-number/agent in one bounded window, and is single-use and frozen. Today it reports **NOT READY with 16 blockers**, and A-L2 is reported **OPEN** rather than quietly satisfied. 118 new offline proofs. [Design §27](ACQUISITION_E7B2_RETELL_DESIGN.md) |
| ~~**E-12I–K**~~ | Final pre-live hardening, local only | **CLOSED LOCALLY — NOTHING PUSHED, DEPLOYED, APPLIED OR SENT.** **E-12I** re-reviewed LPM4, which E-12F had written in the same breath as the code needing it. Changed: the DO block took the first of several matching CHECK constraints with `limit 1`, and `provider_resources` has about a dozen — one schema change from dropping the wrong one. It now counts and refuses. Found: LPM3 does not omit the `client_id` foreign key, it **DEFERS** one and records the statement **with `ON DELETE CASCADE`** — so a real client slugged `aida-acquisition`, later deleted, would cascade our provisioning record away. Preflight (15) and verifier (16) both check that collision; the verifier also checks all six original purpose values **by name**, since losing one breaks receptionist or onboarding provisioning. **Verdict: SAFE TO APPLY TO DEV — and APPLIED + VERIFIED on DEV 2026-08-15.** The purpose CHECK now carries all eight values under the name `provider_resources_purpose_check`, it is the only constraint mentioning `purpose`, the one-agent index and the other constraints are untouched, and the table still holds 0 rows. Verified by reading `pg_get_constraintdef` directly, because the migration reported success with its NOTICEs suppressed — indistinguishable from a no-op. **E-12J** built six negative probes as data for the not-yet-public webhook. Every probe expects 4xx; a 2xx is judged **critical**; all-404 is reported **DORMANT, not a pass**. It deliberately **cannot** prove the positive case — forging a signature would mean writing a second copy of the verifier and congratulating ourselves when our forgery matched our forger. Writing the target validator found a real bug in its own first version: `new URL("https://[::1]").hostname` returns `[::1]` **with brackets**, so the IPv6 loopback was being accepted as a public target. **E-12K** answered the queue question, and the answer is stronger than the gate that was going to be built: **the dial executor is not reachable from the running server at all** — 105 modules reachable from `server.js`, and `acquisition-dial-execution.js` is not among them; no scheduler runs work without a request. `selectNext` is reachable but a **reservation is not a call**. So *enabled + queue 0 + one founder proof* is sufficient because there is **no consumer**, and `assessQueueDrainRisk()` recomputes that from the module graph rather than trusting the claim. Provider activation is likewise a **code change, not a flag flip**: `live:false` is a literal, the provider reads no env var, and a transport must be passed by a caller — so a staging API key cannot become a call transport. Seven-phase runbook; phase 3 checks the queue **before** enabling and uses compare-and-set on the revision; phase 6 requires the paused state restored and the one-shot authority consumed. **No `--execute` flag exists**, deliberately. 91 new offline proofs. [Design §28](ACQUISITION_E7B2_RETELL_DESIGN.md) |
...
| `laq6_bind_provider_call_once.sql` | **applied** 2026-08-13 (E-9) | **not applied** |
