...
| [RETELL_SANDBOX_VALIDATION_PLAN.md](RETELL_SANDBOX_VALIDATION_PLAN.md) | 📐 | The **not yet executed** procedure for first real Retell validation, plus the implemented `scripts/retell-web-sandbox.js` runner: why web calls are gated separately from telephone calls, every sandbox gate, resource order, browser path and its limits, the three separate proofs (API / dashboard audio / full browser), manifest, cleanup, expected cost | Future sandbox procedure, provider cost expectations |
