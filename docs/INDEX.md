...
| **Work on locksmith prospect acquisition** | [OUTBOUND_BDM_ARCHITECTURE.md](OUTBOUND_BDM_ARCHITECTURE.md) (the law and the gates) → [LOCKSMITH_ACQUISITION_SPEC.md](LOCKSMITH_ACQUISITION_SPEC.md) (what was built) |
...
| [LOCKSMITH_ACQUISITION_SPEC.md](LOCKSMITH_ACQUISITION_SPEC.md) | ✅📐 | **A1 prospect intelligence + evidence foundation** — the hardcoded offline boundary (no crawler, no search/directory/DNCR API, enforced in config, registry and schema), acquisition vocabulary, official-source classification, append-only evidence ledger, append-only hash-chained decision log (G17), prospect lifecycle whitelist, discovery adapter contract + deterministic fixture, human review of source and context. Built dormant; SQL written, **not applied**; no site crawled, no external API called, no call placed | Offline boundary, acquisition vocabulary, source authority tiers, evidence ledger, decision log, prospect lifecycle, discovery contract, review step |
