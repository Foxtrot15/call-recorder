...
| [ACQUISITION_E9_CALL_BINDING_DESIGN.md](ACQUISITION_E9_CALL_BINDING_DESIGN.md) | 📐⏳ | **DESIGN ONLY — the constraint does NOT exist, no migration file is written, nothing is applied.** Why one Retell `call_id` must belong to at most one acquisition dispatch. The audit found more than the known read-then-write non-atomicity: for **two different dispatches claiming one `call_id` there is no application check at all**, and `provider_ref` is absent from the laq5 guard's immutable list, so `R1 → R2` is refused by application code and by nothing else. Proposes `unique (provider_ref) where provider_ref is not null`, deliberately **not** partial on `resolved_at` like laq5's other two unique indexes — a business may be dispatched again, but a `call_id` names one real telephone call for ever. Records that the **migration alone is insufficient**, because `updateDialExecution` wraps errors through `fail()` and discards `code`/`constraint`, so a permanent conflict would reach an operator as a transient-sounding `store_unavailable`; and that a **true two-process race would cost a permanent row plus mutating the approved proof row**, so it is offered as a decision rather than taken | provider_ref race, proposed uniqueness, service error mapping, proof residue options |
