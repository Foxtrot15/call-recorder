...
| [ACQUISITION_E9_CALL_BINDING_DESIGN.md](ACQUISITION_E9_CALL_BINDING_DESIGN.md) | ✅📐 | **CLOSED ON DEV 2026-08-13 — `laq6_bind_provider_call_once.sql` applied by hand, verifier 8/8 PASS; production NOT applied.** Read in three layers: §1–8 Phase 1 audit, §9–15 Phase 2 implementation, §16–18 what was actually applied. §16.1 records honestly that the editor did not surface the mutation-probe `RAISE NOTICE` lines, so the six per-case results were **not visually observed** — only that the rollback script completed without SQL error. §16.2 separates what was independently re-verified read-only (census, the unresolved proof dispatch, NULL `provider_ref`, zero duplicates) from what rests on the founder's verifier (the index and guard themselves — PostgREST cannot reach `pg_class`/`pg_proc`, probed not assumed). §17 records the deliberate proof choice: real constraint + rollback probes + offline concurrency tests, **no live two-process race and none claimed**, zero permanent residue. §9 records the founder revision that caught the gap in §4: **a unique index cannot catch the same-dispatch race**, because two workers both reading NULL and wanting *different* references collide with nothing. So laq6 enforces **two** invariants — cross-dispatch uniqueness **and** `provider_ref` being write-once, refusing `R1→R2` **and `R1→NULL`** (without the second, a reference could be cleared and rebound elsewhere while every statement satisfied the index). §10 records why the key is `provider_ref` alone and not `(provider, provider_ref)`: the composite is only sound if `provider` is *also* sticky, and no path can write `provider` at all. §12 records the change without which the migration would mislead — `fail()` discarded `code`/`constraint`, so a permanent conflict reached operators as a transient `store_unavailable`. §15 states plainly that **no live two-process race was run and none is claimed** | provider_ref race, the two invariants, service error mapping, zero-residue proof design |
| [ACQUISITION_E7B2_RETELL_DESIGN.md](ACQUISITION_E7B2_RETELL_DESIGN.md) **§19 = E-10A** | ✅📐⏳ | **§19 specifies the acquisition agent locally §20 splits it into the two Retell resources it actually is, §21 records the first authorised network write (the **response engine is PROVISIONED** on dev while the **agent is not**), and §22 builds the acquisition webhook ingress — a **dedicated, signature-verified, triple-flag-gated** route, **mounted in source but not deployed**, and **LPM3 is now applied to dev**, so the durable idempotency authority — `provider_webhook_events` with UNIQUE `pwe_fingerprint_key` — is present (production has not received it). §22.5 also carries a name correction: the third LPM3 table is `provider_resources`, never `retell_resources`** — opening, prompt, Retell analysis fields and the exact unsent `create-agent` payload. AI disclosure is unprompted and answered plainly if asked, recorded as **founder product policy, not a legal requirement**. Three findings reported rather than smoothed over: the identity model is settled — Aida is the assistant, **Niche Drops** is the calling business, **AIDA** is the product, as three separate fields (§19.2), with ratchets testing **meaning rather than copy** so the opening stays founder-tunable (§19.2a); the marketing tagline is an **absolute** and is **forbidden as a spoken claim**, because copy on a page somebody chose to visit is not a promise made down a telephone to a stranger (§19.3); and **price is never hardcoded or raised first** (§19.4). **Voicemail: leave no message** — it costs a counted attempt, none has been reviewed, and machine detection has never been observed working (§19.5). **§23 = E-12A** makes that policy a **provider setting** rather than an intention: the acquisition agent carries `voicemail_option: { action: { type: "hangup" } }`, so Retell ends the call itself without consulting the model. The finding behind it is that the policy was previously enforced by **nothing on the wire** — `VOICEMAIL_POLICY` is read by documents and tests, and the prompt never carried a word of it. It is a frozen constant, **not** caller-configurable, emitted from **exactly one file**, and the receptionist and onboarding agents are untouched. **Configured locally: YES. Observed on a real call: NO** — and that split also repaired a create-agent blocker that was unsatisfiable by construction. **The response engine is deliberately left unchanged** (founder decision): the prompt is not the machine-detection authority, and a live `updateResponseEngine` write is not justified by duplicate wording — verified byte-identical, so there is no local/remote drift. §23.4a separates the two gates that had been conflated: creating the agent *resource* needs only llm_id, voice and webhook, while the **number, DNCR-1, A-L2, transport and enabling calling are pre-live-call gates** — DNCR-1, A-L2 and E-7 all remain OPEN. IVR and call screening deliberately not enabled. **§24 = E-12B** pins the provisioned response engine by hash (`b0b5e21e…`, using the repo's existing `payloadHash`/`stableStringify`) so the prompt can no longer drift *silently* away from the live `llm_111ed…` — the copy stays tunable, but changing it now forces the remote update it always required, proven by planting real drift and reverting it. §24 also records **one read-only `GET /list-voices`** (272 voices, zero writes): **only 2 carry Australian accent metadata and both are male**, there is **no female preset with an Australian accent**, and the existing receptionist default `custom_voice_018b4225…` is present as "Sunny - Australian Female" but has **no accent/gender/age metadata at all** — reported, not selected. The founder then **auditioned and selected "Sunny - Australian Female"** (§24.7), with the accent recorded as **audition evidence rather than provider metadata**, since Retell supplied none. It lives in a **new acquisition-only key**, `RETELL_ACQUISITION_VOICE_ID` — deliberately not the shared `RETELL_DEFAULT_VOICE_ID`, which holds the same voice today and would have joined the two products by accident, so that re-voicing the receptionist silently re-voiced every cold call. No fallback: an unset key leaves acquisition voiceless, not borrowed. **Voice is no longer a create-agent blocker — `webhook_url` is the only one left.** · **The Retell acquisition provider — E-7B2A COMPLETE (offline adapter + audit), E-7B2B NOT STARTED, E-7 STILL OPEN, calling still PAUSED.** Records the finding that reframed the milestone: `call-recorder` is **not a second repository** — both working copies share one remote, so the boundary is cross-**branch**, and `retell-adapter.js`, `voice-platform-port.js`, `config/retell.js` and the webhook handler are **already on the acquisition branch**. **No cross-repo change was required and none was made. No SQL is required** — laq5's `provider_ref` already carries a Retell `call_id`, and `recordProviderResult` already refuses to set `resolved_at`. Documents the conflict it had to resolve: the shared voice port marks `provider_timeout` **retryable**, which is right for a consenting client and is exactly how a cold acquisition call gets placed twice — so the acquisition provider **discards `retryable` entirely** and RAISES ambiguity instead, yielding `provider_status: unknown`, no `submitted_at`, and the dispatch left **unresolved** for a human. States the request and response contracts, the webhook vocabulary (and that `not_interested` / `declined` / `opt_out` / `callback` are **not** disconnection reasons — every one of them ends in `user_hangup`, so they need post-call analysis, which needs an analysis schema, which needs a new agent), why a **third, purpose-built acquisition agent** is required rather than the inbound receptionist or the onboarding interviewer, and the env names E-7B2B will need. **§8 records the gap a founder check caught before push and the fix that closed it**: the provider originally carried only `executionId`, which is a ONE-WAY 20-hex truncation of `sha256(dispatchId)`, so a lost response left a webhook nothing could be matched to except by recomputing the executor's private hash across every unresolved row. The **exact LAQ5 `dispatchId`** now travels as `aida_dispatch_id`, unhashed and untruncated, `aida_execution_id` is retained beside it for logs, a payload **cannot be built without the durable key**, and a ratchet rejects the executionId, the authorisationId, a hash and a truncation as substitutes. **§14 adds E-7B2B1, the offline return path**: verified event → direct lookup on the exact dispatch id → call-id binding → classification → durable outcome → and only then the lock. It reuses the existing signature verification and `provider_webhook_events` fingerprint idempotency rather than building second ones, needs **no SQL** (`provider_ref` is mutable while `resolved_at` is null, so a lost response reconciles), exposes **no route**, and records the gap it found — **nothing in the pipeline made a prospect contactable**, so the return path would have refused every outcome — **since closed by E-8** (register §14): the dispatch claim establishes `queued` and never `attempted`, a definite provider acceptance or a later authenticated webhook establishes `attempted`, and only the analysis reporting a person was reached establishes `connected` | Retell provider contract, ambiguity semantics, dispatch correlation, agent separation, outcome return path, E-7B2B requirements |
