...
| [RETELL_INBOUND_WEBHOOK_SPEC.md](RETELL_INBOUND_WEBHOOK_SPEC.md) | ✅📐 | **M7F-A secure inbound webhook foundation** — the `retell-sdk` decision and the Node 20 floor it forces, real signature verification, the dedicated `POST /webhooks/retell/inbound` route and why it is separate from the event webhook, the documented request/response contract, what "fail closed" means when a 4xx would disconnect a caller, dynamic-variable and phone-speech reuse, latency and the deliberate absence of any database dependency, inbound URL rules, and a prepared-but-unexecuted Railway sandbox plan. **No number, no call, no exposed webhook, no SQL, no deploy** | Inbound webhook contract, route separation, inbound failure policy, inbound URL construction, sandbox deployment plan |
| [RETELL_SANDBOX_DEPLOYMENT_PLAN.md](RETELL_SANDBOX_DEPLOYMENT_PLAN.md) | ✅📐 | **M7F-B1 Railway sandbox plan** — the Node version decision and its evidence (Node 20 is EOL, so 22 is pinned, not 20 and not the developer's 24), isolated service and project layout, required and explicitly forbidden environment variables, whether the proof needs a database and the two-step recommendation, ordered verification, rollback and cost. **Prepared, not executed: nothing deployed, no endpoint, no DNS, no tunnel** | Sandbox deployment procedure, Node pin rationale, sandbox environment boundary |
| [RETELL_SQL_APPLICATION_CHECKLIST.md](RETELL_SQL_APPLICATION_CHECKLIST.md) | ✅ | **Founder checklist for the Retell provisioning SQL** — the four tables it creates, RLS-at-birth with no policies, idempotency and reversibility, which runtime paths need it, verification queries, rollback, and why `call_inbound` deliberately does **not** depend on it. **Reviewed as text only; not applied** | Retell SQL application procedure, verification queries, rollback |
...
| [LOCKSMITH_SETUP_JOURNEY_SPEC.md](LOCKSMITH_SETUP_JOURNEY_SPEC.md) | ✅📐 | **M8A launch-ready onboarding journey** — the audit showing that nothing but an operator-pasted transcript could create a first draft, the eleven-stage journey, the step declaration that the form and the future voice agent both read, the working-draft lifecycle (a `status:"draft"` row — no new table), contradiction detection, review/approval on the setup surface and why it is not on the M2 session-keyed page, version history and rollback, activation gating, the 320px and accessibility contract, and the founder walkthrough. Built dormant; **no new SQL, nothing deployed, no customer onboarded, no call placed** | Setup step declaration, working-draft lifecycle, contradiction detection, typed-setup review/approval, rollback contract, voice-teaching compatibility |
...
| [../supabase/sql/m7k_add_enquiry_notification_state.sql](../supabase/sql/m7k_add_enquiry_notification_state.sql) | ✅📐 | **M7K notification bookkeeping** — adds the `sending` claim state (the double-send guard: one conditional UPDATE, exactly one winner), plus attempt count, provider, provider reference and failure code. Additive and reversible; existing rows stay `pending`. **Not yet applied; apply after m7j** | Enquiry notification state machine, double-send guard |
| [../supabase/sql/m7j_create_locksmith_enquiries.sql](../supabase/sql/m7j_create_locksmith_enquiries.sql) | ✅📐 | **M7J caller enquiry capture** — creates `locksmith_enquiries` with RLS-at-birth, an `environment` column separating sandbox from production, and a unique index on (client, environment, idempotency_key) that is the duplicate-job guard. Distinct from the landing-page pilot form in `services/locksmith-enquiry.js`, which is a different thing entirely. **Not yet applied; apply after lpm3** | Caller enquiry schema, enquiry idempotency guard |
| [../supabase/sql/m7i_cutover_receptionist_agent.sql](../supabase/sql/m7i_cutover_receptionist_agent.sql) | ✅📐 | **M7I agent cutover** — repoints the single active `provider_resources` receptionist_agent row from the old contract-test agent to the tool-free production-compiled one. Guarded (refuses on zero or multiple matches, asserts exactly one row written), with verification, blast-radius check and a mirrored rollback. Touches no profile, so transfer configuration is untouched. **Review only — not applied** | M7I registry cutover + rollback procedure |
